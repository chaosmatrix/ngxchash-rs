...
impl fmt::Debug for ServerS {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "ServerS {{server: {}, peer: {}, weight: {}, hit_count:{}, hit_rate:{} }}",
...
impl fmt::Debug for HashPointS {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "HashPointS {{server: {:-15},hash: {:-10}}}",
            self.server, self.hash
        )
    }
}
    ...
    match hostport.rfind(":") {
        ...
        _ => {
            return Err(Box::new(MessageError {
                err_msg: fmt::format(format_args!("can't parse host port from '{}'", hostport)),
            }));
        }
    };
}
...
pub fn find_hash_point(hashs: &Vec<HashPointS>, hash: &HashPointS) -> HashPointS {
    match hashs.binary_search(hash) {
        Ok(index) => hashs[index % hashs.len()].to_owned(),
        Err(index) => hashs[index % hashs.len()].to_owned(),
    }
}
...
pub fn ngx_find_peer(upstream: &mut Vec<ServerS>, server_name: &String) -> ServerS {
    let mut server: ServerS = ServerS {
        ..Default::default()
    };
    // TODO
